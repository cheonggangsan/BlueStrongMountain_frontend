...
import { searchMembers } from "../../data/memberStore";
...
async function handleSearchMembers() {
  ...
  try {
    ...
    // 이미 선택된 멤버는 검색 결과에서 "선택됨" 표시만 하고 그대로 노출
...
async function handleSubmit() {
  ...
  try {
    ...
    // ✅ 지금은 Mock store를 통해 생성
    await createGroup(payload);

    // ⚠️ 기억용 주석:
    // 실제 백엔드 붙이면 여기서 createGroup 대신
    // axios.post("/api/v1/groups", payload) 호출 후
    // 응답값으로 groupStore 갱신하면 됨.

    router.push({ name: "GroupList" });
  } catch (e) {
    console.error(e);
    formError.value = "스터디 그룹 생성 중 오류가 발생했습니다.";
  } finally {
...
<template>
  <div class="p-4 max-w-3xl mx-auto">
    <h1 class="text-xl font-bold mb-4">스터디 그룹 생성</h1>
